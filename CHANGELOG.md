...
- **Label scoping now covers BOTH Beeper label systems.** Official Beeper labels (the ones the Desktop app renders) turned out to be Matrix `m.space` rooms flagged `com.beeper.label: true` — not the account-data event `update_label` writes. `getLabelData` now enumerates official labels via `/v1/labels` plus their child state, alongside the legacy store, tagging every label `source: "official" | "legacy"`; `MCP_LABEL_ALLOW` titles match across both and union. Net effect: an instance can be fenced to an app-visible label the user curates in the UI, instead of only to invisible legacy entries. `list_labels` output and chats' `labels[]` include both. (`update_label` deliberately still writes only the legacy store and refuses official labels loudly; the Desktop proxy/CLI hide the Matrix state-write route, but the underlying homeserver accepts it with the native Matrix token — see `docs/labels.md`.)
- **`docs/labels.md`**: the full reverse-engineered reference for both systems — createRoom recipe for official labels (creation content, `via` gotcha for the `labels[]` projection, no-PL rule), children-are-create-time-only, leave-to-delete, plan caps not client-enforced, and the account_data shape for legacy labels.
- **Third MCP instance: label-scoped work agent on `:23377`.** `entrypoint.sh` now launches an optional third server for a remote or low-trust agent: mode from `MCP_TOOL_MODE_WORK` (default `read-only`), chat filter from `MCP_LABEL_ALLOW_WORK`, gated by its own `MCP_WORK_TOKEN`, with a distinct sent ledger. Fail-closed at the launcher: with a token set but no scope, the instance refuses to start rather than run unrestricted. Compose publishes it on loopback + the meshnet address (first consumer: a Preface work agent scoped to a `Preface Public` label, reaching it over Tailscale from another host). Enforcement note documented in `beeperbox.context.md`: the fence holds only while the agent can't read the container env — keep the write token and `BEEPER_TOKEN` off low-trust boxes.
