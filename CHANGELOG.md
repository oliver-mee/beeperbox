...
- **Label scoping now covers BOTH Beeper label systems.** Official Beeper labels (the ones the Desktop app renders) turned out to be Matrix `m.space` rooms flagged `com.beeper.label: true` — not the account-data event `update_label` writes. `getLabelData` now enumerates joined label-spaces alongside the legacy store, tagging every label `source: "official" | "legacy"`; `MCP_LABEL_ALLOW` titles match across both and union. Net effect: an instance can be fenced to an app-visible label the user curates in the UI, instead of only to invisible legacy entries. `list_labels` output and chats' `labels[]` include both. (`update_label` deliberately still writes only the legacy store and refuses official labels loudly — the state proxy is GET-only, so space rosters can't be appended; see new `docs/labels.md`.)
